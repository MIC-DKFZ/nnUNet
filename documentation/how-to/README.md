# How-to Guides

This section groups the existing documentation by task.

## Installation and setup

- [Installation and setup](../getting-started/installation-and-setup.md)

## Data preparation

- [Prepare a dataset](prepare-a-dataset.md)
- [Convert Medical Segmentation Decathlon datasets](../convert_msd_dataset.md)
- [Create manual data splits](../manual_data_splits.md)

## Workflow

- [Plan and preprocess](plan-and-preprocess.md)
- [Train models](train-models.md)
- [Find the best configuration](find-best-configuration.md)
- [Run inference](run-inference.md)

## Training options and related topics

- [Use residual encoder presets](../resenc_presets.md)
- [Run benchmarks](run-benchmarks.md)
- [Pretraining and finetuning](../pretraining_and_finetuning.md)

## Specialized workflows

- [Region-based training](../region_based_training.md)
- [Learning from sparse annotations with ignore labels](../ignore_label.md)
- [Logging reference](../reference/logging.md)

## Advanced customization

- [Modify nnU-Net configurations](../explanation_plans_files.md)
- [Extend nnU-Net](../extending_nnunet.md)
