# Reference

This section collects factual documentation: formats, paths, and configuration details.

## Data and paths

- [Dataset and input format reference](dataset-format.md)
- [Preprocessed data format](preprocessed-data-format.md)
- [Installation and setup](../getting-started/installation-and-setup.md)
- [Setting up paths](../setting_up_paths.md)
- [How to set environment variables](../set_environment_variables.md)

## Configuration and internals

- [Plans and configuration reference](plans-and-configuration.md)
- [CLI overview](cli-overview.md)
- [Intensity normalization](../explanation/intensity-normalization.md)
- [Logging reference](logging.md)

## Operational references

- [Residual Encoder Presets in nnU-Net](../resenc_presets.md)
- [Run benchmarks](../how-to/run-benchmarks.md)
