...
class DataLoader3DGuided(DataLoader3D):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            # save_nifty("/gris/gris-f/homelv/kgotkows/datasets/nnUnet_datasets/nnUNet_raw_data/Task77_frankfurt3Guided/tmp/{}_mask.nii.gz".format(name), seg[j, 0])
...
class DataLoader3DGuided2(DataLoader3D):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            guiding_mask = np.pad(case_all_data[-1:], ((0, 0),
                                                      (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                      (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                      (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                               'constant', **{'constant_values': -1})
            ...
            # save_nifty("/gris/gris-f/homelv/kgotkows/datasets/nnUnet_datasets/nnUNet_raw_data/Task77_frankfurt3Guided/tmp2/{}_mask.nii.gz".format(name), seg[j, 0])
...
class DataLoader3DGuided3(DataLoader3D):

    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None, train_mode=True, deep_i_geos=False, deep_i_geos_value=None):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            # guided_mask_time = 0
            if self.train_mode:
                # Take patch from the ground truth
                guiding_mask = np.pad(case_all_data[-1:], ((0, 0),
                                                          (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                          (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                          (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                ...
                # guided_mask_time = time.time()
                if not self.deep_i_geos:
                    # Convert ground truth into random recommended slices/regions
                    # seg[j, guiding_mask_index] = comp_slices_mask_training(guiding_mask.squeeze(0))[np.newaxis, ...]
                    seg[j, guiding_mask_index] = comp_slices_mask_training(guiding_mask.squeeze(0))[np.newaxis, ...]
                else:
                    # DeepIGeos
                    seg[j, guiding_mask_index] = comp_slices_mask_training(guiding_mask.squeeze(0))[np.newaxis, ...]
                    geodesic_map = seg[j, guiding_mask_index].astype(np.uint8)
                    # geodesic_map[geodesic_map < 0] = 0
                    geodesic_data = data[j][0].astype(np.float32)
                    seg[j, guiding_mask_index] = GeodisTK.geodesic3d_raster_scan(geodesic_data, geodesic_map, properties["spacing_after_resampling"].astype(np.float32), self.deep_i_geos_value, 1)
                # guided_mask_time = time.time() - guided_mask_time
            else:
                guiding_mask = np.pad(case_all_data[-2:-1], ((0, 0),
                                                                           (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                                           (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                                           (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                                                    'constant', **{'constant_values': -1})
            ...
            #         utils.save_nifty(path, data[0, i, ...])
            #     for i in range(2):
            ...
            #         utils.save_nifty(path, seg[0, i, ...])
            # print("")

            # print("Generate batch time: {}, guided time: {}, without guided time: {}".format(time.time() - start_time, guided_mask_time, time.time() - start_time - guided_mask_time))
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys}


class DataLoader3DGuided3_DeepIGeos(DataLoader3D):

    def __init__(self, data, patch_size, final_patch_size, batch_size, num_input_channels, num_classes, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None, train_mode=True, deep_i_geos_value=None):
        self.num_input_channels = num_input_channels
        self.num_classes = num_classes
        super(DataLoader3DGuided3_DeepIGeos, self).__init__(data, patch_size, final_patch_size, batch_size, has_prev_stage,
                 oversample_foreground_percent, memmap_mode, pad_mode, pad_kwargs_data,
                 pad_sides)
        self.train_mode = train_mode
        self.deep_i_geos_value = deep_i_geos_value
    ...
    def determine_shapes(self):
        ...
        num_color_channels = case_all_data.shape[0] - 2
        data_shape = (self.batch_size, num_color_channels, *self.patch_size)
        seg_shape = (self.batch_size, num_seg + self.num_classes - 1, *self.patch_size)
        return data_shape, seg_shape
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            if self.train_mode:
                ...
                # guided_mask_time = time.time()
                # DeepIGeos
                guiding_mask = comp_slices_mask_training(guiding_mask.squeeze(0))[np.newaxis, ...]
                # guided_mask_time = time.time() - guided_mask_time
            else:
                guiding_mask = np.pad(case_all_data[-2:-1], ((0, 0),
                                                                           (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
            ...
            seg_onehot = np.zeros((self.num_classes - 1, *guiding_mask.shape[1:]), dtype=data.dtype)
            for i, l in enumerate(range(1, self.num_classes)):
                seg_onehot[i][guiding_mask[0] == l] = 1
                seg_onehot[i] = GeodisTK.geodesic3d_raster_scan(data[j][0].astype(np.float32), seg_onehot[i].astype(np.uint8), properties["spacing_after_resampling"].astype(np.float32), self.deep_i_geos_value, 1)
                seg_onehot[i] = utils.normalize(seg_onehot[i])
            seg[j, 1:] = seg_onehot
...
class DataLoader3DGuided3_P_Net(DataLoader3D):
    ...
    def __init__(self, data, patch_size, final_patch_size, batch_size, num_input_channels, num_classes, has_prev_stage=False,
        ...
        self.num_input_channels = num_input_channels
        self.num_classes = num_classes
        super(DataLoader3DGuided3_P_Net, self).__init__(data, patch_size, final_patch_size, batch_size, has_prev_stage,
                 oversample_foreground_percent, memmap_mode, pad_mode, pad_kwargs_data,
                 pad_sides)
        self.train_mode = train_mode
        self.deep_i_geos_value = deep_i_geos_value
    ...
    def determine_shapes(self):
        ...
        num_color_channels = case_all_data.shape[0] - 2
        data_shape = (self.batch_size, num_color_channels, *self.patch_size)
        seg_shape = (self.batch_size, num_seg + self.num_classes, *self.patch_size)
        return data_shape, seg_shape
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            if self.train_mode:
                ...
                prediction = np.pad(case_all_data[-2], ((0, 0),
                                                          (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                          (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                          (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                                   'constant', **{'constant_values': -1})
                ...
                guiding_mask = np.pad(case_all_data[-3], ((0, 0),
                                                                           (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                                           (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                                           (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                                                    'constant', **{'constant_values': -1})
            ...
            if seg_from_previous_stage is not None:
                raise RuntimeError("seg_from_previous_stage was not supposed to happen")
                seg[j, 1] = np.pad(seg_from_previous_stage, ((0, 0),
                                                             (-min(0, bbox_x_lb),
                                                              max(bbox_x_ub - shape[0], 0)),
                                                             (-min(0, bbox_y_lb),
