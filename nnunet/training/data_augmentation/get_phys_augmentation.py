...
from nnunet.training.data_augmentation.physical_augmentation import GRESynthesis, InversionRecoverySynthesis, InvGREAugMixedTransform
...
def get_moreDA_inv_augmentation(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
                            border_val_seg=-1,
    ...
    inv_trans = InversionRecoverySynthesis(p_inversion=1.0)
    gre_trans = GRESynthesis(p_gre=1.0)
    mixed_trans = InvGREAugMixedTransform(inv_trans, gre_trans, p_transform=0.7,
                                          p_inv=inv_gre_ratio)
    tr_transforms = [mixed_trans]
