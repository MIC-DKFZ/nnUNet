...
class P_Net_Covid(SegmentationNetwork):
    def __init__(self, patch_size, in_channels=2, out_channels=32, num_classes=2, weightInitializer=InitWeights_He(1e-2), deep_supervision=False, conv_op=None):  # or out_channels = 16/64
        super(P_Net_Covid, self).__init__()
        ...
        self.block6 = nn.Sequential(
          nn.Conv3d(in_channels=out_channels, out_channels=out_channels, kernel_size=[1, 3, 3], stride=1, padding=0, dilation=6), # or kernel_size=[3, 3, 1]
          nn.ReLU(),
          nn.Conv3d(in_channels=out_channels, out_channels=out_channels, kernel_size=[1, 3, 3], stride=1, padding=0, dilation=6),
          nn.ReLU(),
          nn.Conv3d(in_channels=out_channels, out_channels=out_channels, kernel_size=[1, 3, 3], stride=1, padding=0, dilation=6),
          nn.ReLU(),
        )
        self.block7 = nn.Sequential(
          nn.Conv3d(in_channels=int(out_channels/4)*6, out_channels=out_channels, kernel_size=3, stride=1, padding=0, dilation=1), # or kernel_size=[3, 3, 1]
          nn.ReLU(),
        ...
        self.upsample7 = nn.Upsample(size=self.patch_size, mode='trilinear', align_corners=False)
    ...
    def forward(self, x):
        ...
        compress6 = self.upsample6(compress6)
        x = torch.cat((compress1, compress2, compress3, compress4, compress5, compress6), dim=1)
        x = self.block7(x)
        x = self.upsample7(x)
        # x = softmax_helper(x)
        return x
