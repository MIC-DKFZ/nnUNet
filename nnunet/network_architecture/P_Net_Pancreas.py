...
class P_Net_Pancreas(SegmentationNetwork):
    def __init__(self, patch_size, in_channels=2, out_channels=32, num_classes=2, weightInitializer=InitWeights_He(1e-2), deep_supervision=False, conv_op=None):  # or out_channels = 16/64
        super(P_Net_Pancreas, self).__init__()
        ...
        #   nn.Conv3d(in_channels=out_channels, out_channels=out_channels, kernel_size=[1, 3, 3], stride=1, padding=0, dilation=4), # or kernel_size=[3, 3, 1]
        #   nn.ReLU(),
        #   nn.Conv3d(in_channels=out_channels, out_channels=out_channels, kernel_size=[1, 3, 3], stride=1, padding=0, dilation=4),
        #   nn.ReLU(),
        #   nn.Conv3d(in_channels=out_channels, out_channels=out_channels, kernel_size=[1, 3, 3], stride=1, padding=0, dilation=4),
        #   nn.ReLU(),
        # )
        # self.block5 = nn.Sequential(
        #   nn.Conv3d(in_channels=out_channels, out_channels=out_channels, kernel_size=[1, 3, 3], stride=1, padding=0, dilation=5), # or kernel_size=[3, 3, 1]
        #   nn.ReLU(),
        #   nn.Conv3d(in_channels=out_channels, out_channels=out_channels, kernel_size=[1, 3, 3], stride=1, padding=0, dilation=5),
        #   nn.ReLU(),
        #   nn.Conv3d(in_channels=out_channels, out_channels=out_channels, kernel_size=[1, 3, 3], stride=1, padding=0, dilation=5),
        #   nn.ReLU(),
        # )
        self.block6 = nn.Sequential( # *5
          nn.Conv3d(in_channels=int(out_channels/4)*3, out_channels=out_channels, kernel_size=3, stride=1, padding=0, dilation=1), # or kernel_size=[3, 3, 1]
          nn.ReLU(),
    ...
    def forward(self, x):
        ...
        x = torch.cat((compress1, compress2, compress3), dim=1)
        x = self.block6(x)
        x = self.upsample6(x)
        # x = softmax_helper(x)
        return x
