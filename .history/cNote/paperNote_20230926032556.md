...
医学图像分割目前由深度卷积神经网络（CNN）主导。然而，每个分割基准似乎都需要专门的架构和训练方案修改才能实现竞争性能。这导致了该领域大量的出版物，这些出版物除了在有限的验证上进行了有限的验证之外，还使得研究人员越来越难以确定方法是否能够实现他们承诺的优越性，超出了他们所展示的有限的场景。医学分割十项全能赛旨在专门解决这个问题：参与者被要求创建一个分割算法，该算法可以横跨10个数据集，对应于人体的不同实体。这些算法可以动态地适应特定数据集的特定性，但只能以完全自动的方式进行。挑战分为两个连续的阶段：1）开发阶段，在该阶段，参与者可以访问7个数据集，以优化其方法，并使用其最终的冻结方法，必须提交相应的7个保留测试集的分割。2）第二阶段在此阶段，对3个先前未公开的数据集评估相同的确切方法。
