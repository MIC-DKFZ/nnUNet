...
**U-Net Cascade**(ji'lian) 为了解决3D U-Net在具有大图像大小的数据集上的这一实际缺点，我们还提出了一个级联模型。因此，首先在下采样图像上训练3D U-Net（阶段1）。然后，该U-Net的分割结果被上采样到原始体素间距，并作为额外的（一个热编码的）输入通道传递到第二个3D U-Net，该第二个三维U-Net在全分辨率的补丁上进行训练（阶段2）。见图1。
