...
2.5后处理在训练数据上形成所有地面实况分割标签的连接分量分析。如果一个类在所有情况下都位于一个连接的组件中，则这种行为被表示为数据集的一般属性。
因此，在相应数据集的预测图像上，除了该类的最大连接分量之外，其他所有分量都被自动去除。
2.6集合和提交为了进一步提高分割性能和稳健性，我们的三个模型中的两个模型的所有可能组合都为每个数据集集合。
对于最终提交，自动选择在训练集交叉验证上获得最高平均前景骰子分数的模型（或集合）。
