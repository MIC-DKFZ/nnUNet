...
$$ L_{dc} = -\frac{2}{|K|}\sum_{k\in K}\frac{\sum_{i\in I}u_k^iv_k}{\sum_{i\in I}u_k^i + \sum_{i\in I}v_k^i}\tag{2} $$
  ...
  其中$u$是网络的softmax输出，$v$是地面实况分割图的单热点编码。$u$和$v$都具有形状$I\times K$，其中$I\in I$是训练补丁/批中的像素数，$K\in K$是类。
...
由于我们训练的基于补丁的性质，所有推断都是基于补丁进行的。由于网络精度朝着补丁的边界降低，因此在跨补丁聚合预测时，我们对靠近中心的体素的权重高于靠近边界的体素。补丁被选择为按补丁大小/2重叠，我们通过沿所有有效轴镜像所有补丁来进一步利用测试时间数据增强。
 ...
 组合拼接预测和测试时间数据增强导致分割，其中通过聚集多达64个预测（在患者中心使用3D U-Net）来获得每个体素的决策。对于测试用例，我们使用从训练集交叉验证中获得的五个网络作为集合，以进一步提高我们模型的稳健性。

### 2.5后处理

在训练数据上形成所有地面实况分割标签的连接分量分析。如果一个类在所有情况下都位于一个连接的组件中，则这种行为被表示为数据集的一般属性。
因此，在相应数据集的预测图像上，除了该类的最大连接分量之外，其他所有分量都被自动去除。

### 2.6集合和提交

为了进一步提高分割性能和稳健性，我们的三个模型中的两个模型的所有可能组合都为每个数据集集合。对于最终提交，自动选择在训练集交叉验证上获得最高平均前景骰子分数的模型（或集合）。
