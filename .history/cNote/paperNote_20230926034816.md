...
**网络拓扑的动态自适应**(Dynamic adaptation of network topologies) 由于图像大小的巨大差异（*肝* 的中值形状为482×512×512，*海马* 为36×50×35），必须为每个数据集自动调整输入补丁大小和每个轴的池化操作数量（因此隐含地调整卷积层的数量），以允许充分聚合空间信息。
