...
from batchgenerators_torch.transforms.intensity.contrast import ContrastTransform, BGContrast
...
from batchgenerators_torch.transforms.utils.pseudo2d import Convert3DTo2DTransform, Convert2DTo3DTransform
...
class nnUNetTrainer_fasterDA(nnUNetTrainer):
    @staticmethod
...
class nnUNetTrainer_fasterDA_5epochs(nnUNetTrainer_fasterDA):
...
class nnUNetTrainer_fasterDA_bg_style_seg_sampling(nnUNetTrainer_fasterDA):
    @staticmethod
    def get_training_transforms(
            patch_size: Union[np.ndarray, Tuple[int]],
            rotation_for_DA: dict,
        ...
        transforms.append(
            SpatialTransform(
                ...
                p_rotation=0.2,
                rotation=rotation_for_DA['x'], p_scaling=0.2, scaling=(0.7, 1.4), p_synchronize_scaling_across_axes=1,
                bg_style_seg_sampling=True  # , mode_seg='nearest'
            )
        )
