...
class nnUNetTrainer_autopet(nnUNetTrainer):
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        mip_axial = torch.cat([torch.max(data[idx], 4, keepdim=True) for idx in range(np.shape(data)[0])], dim=0)
        mip_coro = torch.cat([torch.max(data[idx], 3, keepdim=True) for idx in range(np.shape(data)[0])], dim=0)
        mip_sagi = torch.cat([torch.max(data[idx], 2, keepdim=True) for idx in range(np.shape(data)[0])], dim=0)
