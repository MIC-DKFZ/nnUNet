...
from nnunetv2.utilities.ddp import AllReduceGrad


@disable  # collectives do not belong in a compiled graph
def global_sum(x: torch.Tensor) -> torch.Tensor:
    """Sum x across all DDP ranks, differentiably. Every rank gets the same result."""
    return AllReduceGrad.apply(x)
