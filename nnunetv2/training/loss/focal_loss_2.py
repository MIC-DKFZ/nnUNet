...
"""
adapted from https://github.com/qubvel/segmentation_models.pytorch/blob/792c2735aa0e926afa17d1105eace63ef2dcc624/segmentation_models_pytorch/losses/focal.py
"""
...
class FocalLoss(_Loss):
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode in {BINARY_MODE, MULTILABEL_MODE}:
            ...
            loss = focal_loss_with_logits(y_pred, y_true, self.gamma, self.alpha, reduction=self.reduction,
                                          normalized=self.normalized, reduced_threshold=self.reduced_threshold,
                                          eps=1e-4)

        elif self.mode == MULTICLASS_MODE:
            assert self.ignore_index is None, 'not implemented'
            y_true = y_true[:, 0].long()
            loss = softmax_focal_loss_with_logits(y_pred, y_true, self.gamma, self.alpha, reduction=self.reduction,
                                          normalized=self.normalized, reduced_threshold=self.reduced_threshold,
                                          eps=1e-4)
