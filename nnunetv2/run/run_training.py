...
def run_training_entry():
    ...
    parser.add_argument('--splits', '-s', required=False, type=str, default=None,
                        help="Specify the path to a custom splits file. It will be copied into the nnUNet_preprocessed folder for the dataset and renamed as splits_final.json.")
    args = parser.parse_args()

    preprocessed_dataset_folder = join(nnUNet_preprocessed, maybe_convert_to_dataset_name(args.dataset_name_or_id))
    splits_file = Path(preprocessed_dataset_folder) / 'splits_final.json'
    if args.splits is None:
        if not splits_file.exists():
            input(" USER INPUT REQUIRED: splits_final.json was not found. One will be autogenerated.\n Press Enter to continue...")
    else:
        if splits_file.exists():
            equal = True
            old_split = load_json(splits_file)
            new_split = load_json(args.splits)
            for fold in range(len(old_split)):
                if sorted(old_split[fold]["train"]) != sorted(new_split[fold]["train"]):
                    equal = False
                if sorted(old_split[fold]["val"]) != sorted(new_split[fold]["val"]):
                    equal = False
            if not equal:
                input(f" splits_final.json already exists and is not equal to {args.splits}.\n Press enter to continue and overwrite the existing splits_final.json")
            else:
                print("WARNING: splits_final.json already exists but equal to the specified splits file. File will be overwritten anyways.")
        shutil.copy(args.splits, splits_file)
