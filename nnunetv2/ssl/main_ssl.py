...
import ssl.ssl_utils as utils

from pathlib import Path
from ssl.desd_loss import DeSDLoss
from ssl.models import DINOHead, nnunet_encoder, DynamicMultiCropWrapper
from ssl.data_loader_ssl import Dataset3D
from ssl.metrics_utils import at_epochs_end, log_example_images
...
def train_ssl(cfg: dict):
    ...
    if 'encoder_lr' in cfg.keys():
        ...
        total_params = sum([np.prod(p.size()) for p in student.parameters()])
        print(f'After freezing - The student has {(params/total_params)*100}% of trainable parameters')
    ...
    if 'encoder_lr' in cfg.keys():
        param_groups = utils.get_params_groups_encoder(student, cfg['encoder_lr'])
        for pg in param_groups:
            optimizer.add_param_group(pg)
        model_parameters = filter(lambda p: p.requires_grad, student.parameters())
        params = sum([np.prod(p.size()) for p in model_parameters])
        total_params = sum([np.prod(p.size()) for p in student.parameters()])
        print(f'After UNfreezin - The student has '
              f'{(params/total_params)*100}% of trainable parameters')
    ...
    if 'encoder_lr' in cfg.keys():
        lr_schedule_enconder = utils.cosine_scheduler(cfg['encoder_lr'] * cfg['batch_size'] / cfg['lr_sch_den'],  # linear scaling rule
                                            cfg['min_encoder_lr'],
                                            cfg['epochs'],
                                            len(train_data_loader),
                                            warmup_epochs=cfg['warmup_epochs'])
    else:
        lr_schedule_enconder = None
    ...
    for epoch in range(start_epoch, cfg['epochs']):
        # ============ training one epoch of SSL ... ============
        train_stats = train_one_epoch(student, teacher, desd_loss, train_data_loader,
                                      optimizer, lr_schedule, lr_schedule_enconder, wd_schedule, momentum_schedule,
    ...
    utils.adapt_chckpt(str(output_dir/'checkpoint.pth'), str(output_dir/'ssl_checkpoint.pth'))


def train_one_epoch(
    student, teacher, desd_loss, data_loader, optimizer, lr_schedule, lr_schedule_enconder,
    ...
    for images in data_loader:
        ...
        for i, param_group in enumerate(optimizer.param_groups):
            if 'enc' in param_group.keys():
                param_group["lr"] = lr_schedule_enconder[it]
            else:
