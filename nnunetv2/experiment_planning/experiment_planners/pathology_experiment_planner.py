...
class PathologyExperimentPlanner(object):
    def __init__(self, dataset_name_or_id: Union[str, int],
                 gpu_memory_target_in_gb: float = 8,
                 preprocessor_name: str = 'DefaultPreprocessor', plans_name: str = 'nnUNetWholeSlideDataPlans',
        ...
        self.dataset_json = load_json(join(preprocessed_folder, 'dataset.json'))
    ...
    #     print('transpose_forward, transpose_backward', transpose_forward, transpose_backward)
...
###
    ...
    def plan_experiment(self):
        ...
        # first get transpose
        transpose_forward, transpose_backward = [0, 1, 2], [0, 1, 2] #self.determine_transpose()
        ...
        # print(fullres_spacing_transposed)
        fullres_spacing_transposed = [999, 1, 1] # Handeled by wholeslidedata
        ...
        # print(new_median_shape_transposed)
        new_median_shape_transposed = self.Unet_max_patch_size if len(self.Unet_max_patch_size)==3 else [1] + self.Unet_max_patch_size 

        approximate_n_voxels_dataset = None # float(np.prod(new_median_shape_transposed, dtype=np.float64) *
        ...
        # 2D configuration
        print('fullres_spacing_transposed[1:]', fullres_spacing_transposed[1:])
        print('new_median_shape_transposed[1:]', new_median_shape_transposed[1:])
        print('self.generate_data_identifier(2d)', self.generate_data_identifier('2d'))
        print('approximate_n_voxels_dataset', approximate_n_voxels_dataset)
        ...
        plans = {
            ...
            'image_reader_writer': "NaturalImage2DIO", # This was returned when loading png's, not sure where this is used next to loading data (carried out by wholeslidedata now), keeping this for now
