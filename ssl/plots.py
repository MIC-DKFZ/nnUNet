    ...
    for exp_cfg in exps_cfgs:
        ...
        if plot_images:
            ...
            if plot_type == 0:
                ...
                set_axis(ax[0, 1])
                # Plot attibutions
                ax[1, 0].set_title('Attibution maps')
                ...
                ax[0, 2].set_title('NCCT Attibution maps')
                ...
                set_axis(ax[0, 2])
                # Plot diff images
                ax[1, 0].set_title('Interhem. difference images')
                ax[1, 0].imshow(diff, cmap='gray', vmin=-50, vmax=50)
                set_axis(ax[1, 0])
                # Plot ncct attibutions
                ax[1, 1].set_title('Diff. Img. Attibution maps')
                ...
                set_axis(ax[1, 2])
        fg.suptitle(f'Enconder embedings for experiment: ' \
                    f'{exp_cfg["data"]} - {exp_cfg["context"]}', fontsize=16)
        plt.tight_layout()
        plt.show()
