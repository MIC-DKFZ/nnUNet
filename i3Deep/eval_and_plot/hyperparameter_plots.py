...
for i, basename in enumerate(basenames):
    ...
    for j, task in enumerate(tasks):
        ...
        for param_key in results.keys():
            ...
            ax.set_ylabel('Dice Score', color=colors[0])
            ax.plot(result_x, result_y_dice, label="Dice", color=colors[0])
            ax.set_ylim(np.min(result_y_dice), np.min(result_y_dice) + diff_max)
            # ax.tick_params(axis='y', labelcolor=colors[0])
            ax2 = ax.twinx()
            ax2.set_ylabel('Annotation Ratio', color=colors[1])
            ax2.plot(result_x, result_y_ratio, label="Annotation Ratio", color=colors[1])
